...
Package tests live next to the code under `internal/` and `cmd/mcpload`. Feature tests live in `features/`.

`make build` writes `bin/mcpload`. Feature acceptance compiles its own binary into a temp dir.
...
Unit tests in `internal/session`, `internal/oauth`, `internal/runner`, and `cmd/mcpload` start these servers and point scenarios at `srv.URL()`.

## Godog

`features/harness_test.go` runs one `godog.TestSuite` named `harness` with `Strict: true` and `Paths: []string{"."}`. `TestFeatures` compiles `./cmd/mcpload` once, then walks every `*.feature` in `features/`.
...
| `acceptance.feature` | Real `mcpload` binary against both fakes. Headers + OAuth, 401 → exit 1. |
...
`cmd/mcpload/main_test.go` drives `run([]string{…})` with swapped stdout/stderr. It checks exit codes for `version`, missing paths, unknown commands, `validate` of the frozen file, `--vus`/`--duration`, `--http-shared-pool`, `--out` / `--out-file`, threshold exit 1, and the insecure-secrets warning.
